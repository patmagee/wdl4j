...
public class ReadLines implements EngineFunction {
    ...
    public Type evaluateReturnType(List<Type> argumentTypes) throws WdlValidationError {
        ...
        if (!argumentType.isCoercibleTo(FileType.getType()) && !argumentType.isCoercibleTo(StringType.getType())) {
            throw new TypeCoercionException(
                    "Illegal argument type for read_lines function, expecting one of [String,File] but got " + argumentType
                            .getTypeName());
        }
    ...
    public void checkArity(List<Type> argumentTypes) throws ArityException {
        if (argumentTypes == null || argumentTypes.size() != 1) {
            throw new ArityException("Invalid number of arguments for function read_lines. Expecting 1 but got " + (
                    argumentTypes == null
                    ? "none"
                    : argumentTypes.size()));
        }
